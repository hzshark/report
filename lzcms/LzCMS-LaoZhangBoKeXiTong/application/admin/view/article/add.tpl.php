    ...
    <div class="layui-tab-content">
       <form class="layui-form">
        <div class="layui-tab-item layui-show">
          ...
          <?php echo Form::input('text','keywords','','关键词','关键词以英文逗号隔开');?>
          <?php echo Form::textarea('description','','摘要','留空时默认截取内容的前250个字符');?>
          <?php echo Form::layedit('content','','内容','','请输入内容','layedit','content');?>
          <?php echo Form::date('create_time',date('Y-m-d H:i:s'),'添加时间','默认是当前时间');?>
          <?php echo Form::input('text','hits',0,'点击量','请输入数字','请输入点击量，默认是0','number');?>
...
layui.use(['element', 'form', 'upload', 'layedit', 'laydate'], function(){
  ...
  form.on('submit(article_add)', function(data){
    ...
    jq.post('{:url("article/add")}',param,function(data){
      if(data.code == 200){
        layer.close(loading);
